```mermaid
flowchart TD
    A['Olá! Bem vindo ao SyncDesk! Para começarmos, verifiquei no seu cadastro e você possui os seguintes produtos disponíveis para manutenção. Selecione a opção que indica sobre o que você quer falar hoje:<br> 1-Produto A<br> 2-Produto B<br> 3-Produto C<br> 4-Desejo apenas tirar uma dúvida.<br> 5-Desejo uma liberação de acesso no Sync Desk.<br><br> **Responda apenas o número da opção**]
    A -->|1| B
    A -->|2| B
    A -->|3| B[Entendi. Como posso te ajudar hoje em relação ao Produto >produto escolhido<? <br> 1-O sistema apresenta falhas.<br> 2-Quero solicitar uma nova função.<br> *Responda apenas o número da opção*]
    A -->|4| C[Entendi. Selecione, por favor, qual a sua dúvida: <br> 1-Qual o período restante para manutenção dos sistemas que eu já adquiri?<br> 2-Estou com dúvidas sobre como utilizar um dos meus sistemas.<br> 3-Como faço para solicitar um novo sistema?<br><br>**Responda apenas o número da opção**]
    ...
    A -->|Opção inválida| Z[Ops! Opção inválida, você será redirecionado para o menu principal.]
    B -->|1| F[Por favor, explique da maneira mais detalhada possível o seu problema. Lembre-se: Se a descrição do problema não for clara e/ou faltarem informações, seus chamado poderá ser cancelado pelo time de suporte. Seja específico e detalhista.]
    ...
    B -->|Opção inválida| Z
    C -->|1| I[Verifiquei e esses são os seguintes prazos:<br> Produto A - Até dd/mm/aaaa<br> Produto B - Até dd/mm/aaa<br> Produto C - Até dd/mm/aaaa]
    ...
    I --> H[Ajudo em algo mais?<br> 1-Sim<br> 2-Não]
    J --> H
    H -->|Sim| A
